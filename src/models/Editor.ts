...
export class Editor {
    ...
    cursor: Cursor;
    private static EDITOR_DEFAULT_CONTENT = '<div data-line-number="1"><br></div>';
    ...
    resetContent(): void {
        this.lines = Array.from(Editor.EDITOR_DEFAULT_CONTENT).map((line, index) => {
            return {
                number: index + 1,
                content: line
            }
    ...
    parseInitialContent(content: string): Array<{ number: number; content: string }> {
    ...
    addLine(index: number, content = '') {
        ...
        this.cursor.moveCursor({ cursorCharacter: 0, cursorLine: this.cursor.position.cursorLine + 1 });
    }
    ...
    insertCharacter(char: string, cursorLine: number, cursorCharacter: number) {
        ...
        if (this.lines.length >= cursorLine) {
            ...
            lineContent = lineContent.substring(0, cursorCharacter) + char + lineContent.substring(cursorCharacter);
            // Update the line content
            this.lines[cursorLine - 1].content = lineContent;
            // Move the cursor forward by one character
            this.cursor.shiftCursor({ deltaChar: 1 });
        }
    }
    ...
    deleteCharacter(cursorLine: number, cursorCharacter: number) {
        if (cursorCharacter > 0) {
            ...
            line.content = line.content.substring(0, cursorCharacter - 1) + line.content.substring(cursorCharacter);
            this.cursor.shiftCursor({ deltaChar: -1 }); // Move cursor back by one character
            ...
            this.cursor.moveCursor({ cursorLine: cursorLine - 1, cursorCharacter: previousLineLength });
        }
    }
    ...
    removeLine(index: number) {
        ...
        this.cursor.shiftCursor({ deltaLine: -1 });
    }
