...
export class Cursor {
    ...
    private setCursorPosition() {
        cursorPosition.set({ cursorLine: this.position.cursorLine, cursorCharacter: this.position.cursorCharacter });
    }

    moveCursor(newPosition: Partial<CursorPosition>) {
        this.position = {
            cursorBasis: newPosition.cursorBasis ?? this.position.cursorBasis,
            cursorCharacter: newPosition.cursorCharacter ?? this.position.cursorCharacter,
    ...
    moveCursorLeft(previousLineContentLength?: number) {
        ...
        if (this.position.cursorCharacter === 0 && this.position.cursorLine === 1) return;
    
        if (this.isAtLineStart()) {
            if (typeof previousLineContentLength === "number") {
                // Move cursor to the end of the previous line
                this.moveCursor({ cursorCharacter: previousLineContentLength, cursorLine: this.position.cursorLine - 1 });
            }
        } else {
    ...
    moveCursorRight(currentLineContentLength: number, totalLines: number) {
        ...
        if (this.position.cursorCharacter === currentLineContentLength && this.position.cursorLine === totalLines) return;

        if (this.isAtLineEnd(currentLineContentLength)) {
            // Move cursor to start of next line
            this.moveCursor({ cursorCharacter: 0, cursorLine: this.position.cursorLine + 1 });
        } else {
            // Move the cursor one character to the right
            this.shiftCursor({ deltaChar: 1 });
        }
    }
    ...
    moveCursorUp(previousLineContentLength: number) {
        ...
        this.moveCursor({ cursorCharacter: Math.min(this.position.cursorCharacter, previousLineContentLength), cursorLine: this.position.cursorLine - 1 })
    }
    ...
    moveCursorDown(nextLineContentLength: number, totalLines: number) {
        ...
        this.moveCursor({ cursorCharacter: Math.min(this.position.cursorCharacter, nextLineContentLength), cursorLine: this.position.cursorLine + 1 })
    }
    
    shiftCursor(adjustment: { deltaChar?: number, deltaLine?: number }) {
        this.position = {
            cursorBasis: this.position.cursorBasis + (adjustment.deltaChar ?? 0),
            cursorCharacter: this.position.cursorCharacter + (adjustment.deltaChar ?? 0),
