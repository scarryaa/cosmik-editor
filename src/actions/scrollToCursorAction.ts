...
export function scrollToCursorAction(
    ...
    function scrollToCursor(event: CustomEvent<{ direction: CursorDirection }>) {
        ...
        if (isNearLeftEdge && params.$editor.getCursor().getDirection() === CursorDirection.Left) {
            node.scrollTo({
                left: cursorLeft - edgeThreshold,
                behavior: "auto",
            });
        } else if (isNearRightEdge && params.$editor.getCursor().getDirection() === CursorDirection.Right) {
            node.scrollTo({
                left: cursorLeft - node.clientWidth + edgeThreshold,
                behavior: "auto",
            });
        } else if (event.detail.direction === CursorDirection.Down) {
            scrollToCurrentLine(params.$currentLineElement, params.$astroWrapperInner, "down");
        } else if (event.detail.direction === CursorDirection.Up) {
            scrollToCurrentLine(params.$currentLineElement, params.$astroWrapperInner, "up");
        }
    }
    ...
    node.addEventListener('cursorPositionChange', (event) => handleCursorPositionChange(event as CustomEvent));

    return {
        destroy() {
            node.removeEventListener('cursorPositionChange', (event) => handleCursorPositionChange(event as CustomEvent));
        },
    };
}