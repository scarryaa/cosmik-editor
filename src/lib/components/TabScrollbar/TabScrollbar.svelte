<script lang="ts">
    ...
    import { openTab, tabs } from "../../reactives/tabs.svelte";
    ...
    onMount(() => {
        ...
        const handleWheel = (event: WheelEvent): void => {
            if (event.shiftKey) {
                ...
                deltaXAccumulator += event.deltaX;
    
                // Check if the accumulated deltaX exceeds the threshold
                if (Math.abs(deltaXAccumulator) >= deltaXThreshold) {
                    const direction = Math.sign(deltaXAccumulator);
                    const currentTabIndex = $tabs.findIndex(
                        (tab) => tab.id === $activeTabId,
                    );
                    let nextTabIndex = currentTabIndex + direction;
    
                    // Ensure the next tab index is within bounds
                    ...
                    const nextTabId = $tabs[nextTabIndex];
                    openTab(nextTabId);
        ...
        const scrollToActiveTab = async () => {
            ...
            if (activeTab && scrollableContent) {
    ...
    <div
        ...
        bind:this={scrollbarThin}
        class:opacity-0={!overflowing}
    >
