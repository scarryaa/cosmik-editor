...
export const highlightJson = (json: string): string => {
    ...
    escapedJson = escapedJson.replace(/\"(.*?)\"/g, (match) => {
        const placeholder = `@@PLACEHOLDER_${placeholderIndex++}@@`;
        placeholdersMap.set(placeholder, `<span class="json-string">${match}</span>`);
        return placeholder;
    });

    escapedJson = escapedJson.replace(/\b\d+\.?\d*\b/g, `<span class="json-number">$&</span>`);
    escapedJson = escapedJson.replace(/\b(false|true|null)\b/g, `<span class="json-boolean">$&</span>`);
