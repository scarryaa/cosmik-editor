...
export const scrollToCurrentLine = (
    ...
    const isNearBottomEdge = (elementTop + lineHeight) > (visibleBottom - edgeThreshold);
    ...
    if (direction === "down" && isNearBottomEdge) {
        ...
        $astroWrapperInner.scrollBy({ top: lineHeight + edgeThreshold - (visibleBottom - elementTop), behavior: "auto" });
    } else if (direction === "up" && isNearTopEdge) {
        // Scroll up to bring the current line fully into view
        $astroWrapperInner.scrollBy({ top: -(visibleTop - elementTop + edgeThreshold), behavior: "auto" });
    }
};
...
export const scrollToCursor = (
    ...
    const contentUpToCursor = $editor.getContent()[cursorPosition.line].content.substring(0, cursorPosition.character);
    ...
    if (isNearLeftEdge) {
        $astroWrapperInner.scrollTo({ left: cursorLeft - edgeThreshold, behavior: "auto" });
    } else if (isNearRightEdge) {
        $astroWrapperInner.scrollTo({ left: cursorLeft - $astroWrapperInner.clientWidth + edgeThreshold, behavior: "auto" });
    }
}